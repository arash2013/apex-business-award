...
def upgrade() -> None:
    ...
    op.add_column("businesses", sa.Column("area_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key(
        "fk_businesses_area_id", "businesses", "areas", ["area_id"], ["id"], ondelete="SET NULL"
    )

    op.add_column("businesses", sa.Column("category_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key(
        "fk_businesses_category_id", "businesses", "categories", ["category_id"], ["id"], ondelete="SET NULL"
    )
...
def downgrade() -> None:
    ...
    op.add_column("businesses", sa.Column("neighborhood", sa.String(100), nullable=True))
