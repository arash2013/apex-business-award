...
async def _send_outreach(outreach_id: UUID) -> None:
    async with AsyncSessionLocal() as db:
        ...
        template = await db.execute(
            ...
            __import__("sqlalchemy", fromlist=["select"]).select(EmailTemplate)
        ...
        logger.info("Sent outreach %s (step %s) to %s", outreach_id, outreach.sequence_step, outreach.email_address)
        ...
        if outreach.sequence_step in FOLLOW_UP_DAYS:
            ...
            _schedule_follow_up(str(outreach.business_id), str(outreach.award_id),
                                outreach.email_address, next_step, delay_days)
...
def _schedule_follow_up(
    ...
    send_outreach_follow_up.apply_async(
        kwargs={"business_id": business_id, "award_id": award_id, "email": email, "step": step},
